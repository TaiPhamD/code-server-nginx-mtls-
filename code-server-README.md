...
## Step 3.1: Access code-server

code-server is now available at `https://<server-ip>/code/` (not the root `/`).

## Step 2.5: Firewall (if using UFW)

Nginx listens on port **443** (HTTPS). If you're accessing from another machine, open the port:
...
If you only access locally, no firewall changes are needed.
